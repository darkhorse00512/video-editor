...
interface NextSignInFormProps {
  onToggleMode?: () => void;
  onSuccess?: () => void;
}

export function NextSignInForm({ onToggleMode, onSuccess }: NextSignInFormProps) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const userResponse = await fetch(
          `${config.supabase_server_url}/user?uuid=eq.${data.user.id}`,
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
        ...
        if (userData && userData.length > 0) {
          ...
          // User data not found in Supabase
  ...
  const handleGoogleSignIn = async () => {
    setIsLoading(true);
    try {
      // TODO: Implement Google OAuth for Next.js
