  ...
  const testSupabaseConnection = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "https://supabase.nymia.io/auth/v1/settings",
        {
          method: "GET",
          headers: {
      ...
      addLog("Environment Check", {
        ...
        redirectUrl: "https://app.nymia.ai/auth/callback",
      });
    } catch (error) {
  ...
  const testGoogleSignIn = async () => {
    ...
    try {
      const { data, error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          redirectTo: `https://app.nymia.ai/auth/callback`,
        },
      });
